...
8. If any changes are made to the database, run this code in the vscode container terminal to backup the database before committing changes or the changes will not be made for everyone
    a. mariadb-dump -h db -u mariadb -p mariadb > backup.sql --skip-ssl
    b. When prompted to enter a password, enter 'mariadb', it will not look like you are typing anything - this is because of shoulder surfing protections in the extention.
